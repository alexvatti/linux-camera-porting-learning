...
static int ov5648_detect(struct ov5648_sensor *sensor)
{
	...
	//dev_info(sensor->dev, "Device ID: 0x%02x%02x\n",chip_id_h,chip_id_l);
...
{
	int ret;
	//dev_info(sensor->dev, "ov5648_sensor_init");
...
static int ov5648_sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	int ret = 0;
	struct ov5648_sensor *sensor = ov5648_subdev_sensor(sd);
	//dev_info(sensor->dev, "ov5648_sensor_power\n");
				    ...
				    const struct ov5648_mode *mode)
{
	...
	mbus_format->xfer_func = V4L2_XFER_FUNC_NONE;
}
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	dev_info (sensor->dev, "ov5648_get_fmt\n");
	mutex_lock(&sensor->mutex);
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	dev_info (sensor->dev, "ov5648_set_fmt\n");
	mutex_lock(&sensor->mutex);
	...
	dev_info (sensor->dev, "target code=%x\n",mbus_format->code);
	...
	if (!mbus_code){
		...
		dev_info (sensor->dev, "setting to default code\n");
	}
	...
	dev_info (sensor->dev, "after fill: mbus_format->code=%x, mbus_code=%x\n",mbus_code,mbus_format->code);
...
static int ov5648_s_power(struct v4l2_subdev *sensor, int on){
	return 0;
}
...
static void ov5648_set_defaults(struct ov5648_sensor *sensor)
{
