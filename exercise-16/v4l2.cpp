...
bool V4L2::read (cv::Mat &img)
{
    ...
    if (m_sensor == V4L2::SENSOR_AR0144){
        ...
        cv::Mat Mat(Sensors[m_sensor].imageHeight, Sensors[m_sensor].imageWidth, CV_8UC1,(unsigned char *)buffers[buff.index].start);
        //cv::cvtColor(Mat, img, cv::COLOR_BayerBG2RGB);
        //cv::cvtColor(Mat, img, cv::COLOR_BayerGR2RGB);
        //cv::cvtColor(Mat, img, cv::COLOR_BayerGB2RGB);
        cv::cvtColor(Mat, img, cv::COLOR_BayerRG2RGB);
    }else if (m_sensor == V4L2::SENSOR_IMX219){
        cv::Mat Mat(Sensors[m_sensor].imageHeight, Sensors[m_sensor].imageWidth, CV_8UC1,(unsigned char *)buffers[buff.index].start);
    }
...
//v4l2-ctl --set-fmt-video=width=1280,height=720,pixelformat=RG10 --stream-mmap --stream-count=1 --device /dev/video0 --stream-to=/root/ov5648-1.raw
