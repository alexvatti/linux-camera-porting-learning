...
bool V4L2::read (cv::Mat &img)
{
    ...
    if (m_sensor == V4L2::SENSOR_AR0144){
        ...
        cv::Mat Mat(Sensors[m_sensor].imageHeight, Sensors[m_sensor].imageWidth, CV_8UC2,(unsigned char *)buffers[buff.index].start);
        cv::cvtColor(Mat, img, cv::COLOR_YUV2RGB_UYVY);
        //cv::cvtColor(Mat, img, cv::COLOR_YUV2RGB_YVYU);
        //cv::cvtColor(Mat, img, cv::COLOR_YUV2RGB_YUYV);
    }else if (m_sensor == V4L2::SENSOR_IMX219){
        cv::Mat Mat(Sensors[m_sensor].imageHeight, Sensors[m_sensor].imageWidth, CV_8UC1,(unsigned char *)buffers[buff.index].start);
    }
