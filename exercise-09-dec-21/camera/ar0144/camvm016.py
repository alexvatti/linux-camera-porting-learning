...
def get_camera():
    ...
    path = os.path.join('/sys/bus/i2c/devices', '1-0010', 'driver')
    if not os.path.exists(path):
        return None

    failed = False
    # Make sure the camera is in a defined state
    cmd = 'media-ctl -V "61:0[fmt:SGRBG8_1X8/1280x800 (4,4)/1280x800]"'
    ret = subprocess.call(cmd, shell=True)
    if ret != 0:
        print('media-ctl failed: {}'.format(ret))
        failed = True
    cmd = 'media-ctl -V "43:0[fmt:SGRBG8_1X8/1280x800]"'
    ret = subprocess.call(cmd, shell=True)
    if ret != 0:
        print('media-ctl failed: {}'.format(ret))
        failed = True
...
cam = get_camera()
mat, ret = cam.read()
cv2.imwrite("/root/sample.jpg",mat)
cam.release()
