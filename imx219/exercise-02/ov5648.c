...
#define OV5648_SW_STANDBY_REG				0x0100
#define OV5648_SW_STANDBY_STREAM_ON			BIT(0)

#define OV5648_SW_RESET_REG					0x0103
...
#define to_ov5648(s) \
	container_of(s, struct ov5648, subdev)

#define ov5648_ctrl_subdev(c) \
	(&container_of((c)->handler, struct ov5648, \
...
struct ov5648_format ov5648_formats[] = {
	{
...
struct ov5648 {
	...
	const struct ov5648_format *formats;
	unsigned int num_fmts;
	const struct ov5648_mode *modes;
...
static const u8 ov5648_test_pattern_bits[] = {
	...
	OV5648_ISP_CTRL3D_PATTERN_EN | OV5648_ISP_CTRL3D_ROLLING_BAR_EN | OV5648_ISP_CTRL3D_PATTERN_COLOR_BARS,
	OV5648_ISP_CTRL3D_PATTERN_EN | OV5648_ISP_CTRL3D_PATTERN_COLOR_SQUARES,
	OV5648_ISP_CTRL3D_PATTERN_EN | OV5648_ISP_CTRL3D_ROLLING_BAR_EN | OV5648_ISP_CTRL3D_PATTERN_COLOR_SQUARES,
};

/* Input/Output */

static int ov5648_read(struct ov5648 *sensor, u16 address, u8 *value)
{
...
static int ov5648_write(struct ov5648 *sensor, u16 address, u8 value)
{
...
static int ov5648_write_sequence(struct ov5648 *sensor,
...
static int ov5648_update_bits(struct ov5648 *sensor, u16 address,
			      u8 mask, u8 bits)
{
	u8 value = 0;
	int ret;
...
static int ov5648_sw_reset(struct ov5648 *sensor)
{
	return ov5648_write(sensor, OV5648_SW_RESET_REG, OV5648_SW_RESET_RESET);
}

static int ov5648_sw_standby(struct ov5648 *sensor, int standby)
{
	u8 value = 0;
...
static int ov5648_chip_id_check(struct ov5648 *sensor)
{
...
static int ov5648_avdd_internal_power(struct ov5648 *sensor, int on)
{
	return ov5648_write(sensor, OV5648_A_PWC_PK_O0_REG,
			    on ? 0 : OV5648_A_PWC_PK_O0_BP_REGULATOR_N);
}

static int ov5648_pad_configure(struct ov5648 *sensor)
{
	int ret;
...
static int ov5648_mipi_configure(struct ov5648 *sensor)
{
...
static int ov5648_black_level_configure(struct ov5648 *sensor)
{
	int ret;
...
static int ov5648_isp_configure(struct ov5648 *sensor)
{
	u8 bits;
	int ret;
...
static unsigned long ov5648_mode_pll1_rate(struct ov5648 *sensor,
...
static int ov5648_mode_pll1_configure(struct ov5648 *sensor,
				      const struct ov5648_mode *mode,
				      u32 mbus_code)
{
	const struct ov5648_pll1_config *config;
...
static int ov5648_mode_pll2_configure(struct ov5648 *sensor,
				      const struct ov5648_mode *mode)
{
...
static int ov5648_mode_configure(struct ov5648 *sensor,
...
static unsigned long ov5648_mode_mipi_clk_rate(struct ov5648 *sensor,
					       const struct ov5648_mode *mode,
					       u32 mbus_code)
{
	const struct ov5648_pll1_config *config;
...
static int ov5648_exposure_auto_configure(struct ov5648 *sensor,
					  bool enable)
{
...
static int ov5648_exposure_configure(struct ov5648 *sensor, u32 exposure)
{
...
static int ov5648_exposure_value(struct ov5648 *sensor,
				 u32 *exposure)
{
...
static int ov5648_gain_auto_configure(struct ov5648 *sensor, bool enable)
{
	return ov5648_update_bits(sensor, OV5648_MANUAL_CTRL_REG,
...
static int ov5648_gain_configure(struct ov5648 *sensor, u32 gain)
{
	struct ov5648_ctrls *ctrls = &sensor->ctrls;
	int ret;
...
static int ov5648_gain_value(struct ov5648 *sensor, u32 *gain)
{
...
static int ov5648_white_balance_auto_configure(struct ov5648 *sensor,
					       bool enable)
{
...
static int ov5648_white_balance_configure(struct ov5648 *sensor,
...
static int ov5648_flip_vert_configure(struct ov5648 *sensor, bool enable)
{
...
static int ov5648_flip_horz_configure(struct ov5648 *sensor, bool enable)
{
...
static int ov5648_test_pattern_configure(struct ov5648 *sensor,
					 unsigned int index)
{
...
static int ov5648_state_mipi_configure(struct ov5648 *sensor,
				       const struct ov5648_mode *mode,
				       u32 mbus_code)
{
	struct ov5648_ctrls *ctrls = &sensor->ctrls;
...
static int ov5648_state_configure(struct ov5648 *sensor,
				  const struct ov5648_mode *mode,
				  u32 mbus_code)
{
	int ret;
...
static int ov5648_state_init(struct ov5648 *sensor)
{
	return ov5648_state_configure(sensor, &ov5648_modes[0],
				      ov5648_mbus_codes[0]);
}

static int ov5648_detect(struct ov5648 *sensor)
{
	...
	ret = ov5648_write(sensor, OV5648_SW_RESET_REG, 0x01);
	if (ret < 0)
		return ret;

	ret = ov5648_read(sensor, OV5648_CHIP_ID_H_REG, &chip_id_h);
	if (ret < 0)
		return ret;
	
	if (chip_id_h != OV5648_CHIP_ID_H_VALUE) {
		dev_err(&client->dev, "ID High expected 0x56 got %02x", chip_id_h);
		return -ENODEV;
	}

	ret = ov5648_read(sensor, OV5648_CHIP_ID_L_REG, &chip_id_l);
	if (ret < 0)
		return ret;
	
	if (chip_id_l != OV5648_CHIP_ID_L_VALUE) {
		dev_err(&client->dev, "ID Low expected 0x48 got %02x", chip_id_l);
		return -ENODEV;
	}

	return ov5648_write(sensor, OV5648_SW_RESET_REG, 0x00);
}

/* Sensor Base */

static int ov5648_sensor_init(struct ov5648 *sensor)
{
	int ret;
...
static int ov5648_sensor_power(struct v4l2_subdev *sd, int on)
{
	/* Keep initialized to zero for disable label. */
	int ret = 0;
	struct ov5648 *sensor = to_ov5648(sd);
...
static int ov5648_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	struct ov5648 *sensor = to_ov5648(subdev);
	struct ov5648_ctrls *ctrls = &sensor->ctrls;
	int ret;
...
static int ov5648_ctrls_init(struct ov5648 *sensor)
{
	struct ov5648_ctrls *ctrls = &sensor->ctrls;
...
#if 0
static int ov5648_stream_on(struct v4l2_subdev *sd)
{
	int ret;

	ret = ov5648_write(sd, OV5648_MIPI_CTRL0_REG, OV5648_MIPI_CTRL0_IDLE_LP11);
	if (ret < 0)
		return ret;

	ret = ov5648_write(sd, OV5648_FRAME_OFF_NUM_REG, 0x00);
	ret = ov5648_write(sd, OV5648_FRAME_ON_NUM_REG, 0x0F);
	
	if (ret < 0)
		return ret;

	return ov5648_write(sd, OV5648_PAD_OUT2_REG, 0x00);
}

static int ov5648_stream_off(struct v4l2_subdev *sd)
{
	int ret;

	ret = ov5648_write(sd, OV5648_MIPI_CTRL0_REG, MIPI_CTRL00_CLOCK_LANE_GATE
			   | OV5648_MIPI_CTRL0_IDLE_LP00 | MIPI_CTRL00_CLOCK_LANE_DISABLE);
	if (ret < 0)
		return ret;

	ret = ov5648_write(sd, OV5648_FRAME_OFF_NUM_REG, 0x0F);
	ret = ov5648_write(sd, OV5648_FRAME_ON_NUM_REG, 0x00);
	if (ret < 0)
		return ret;

	return ov5648_write(sd, OV5648_PAD_OUT2_REG, 0x01);
}
...
{
	if (code->index >= ARRAY_SIZE(ov5648_mbus_codes))
		return -EINVAL;

	code->code = ov5648_mbus_codes[code->index];
				    ...
				    const struct ov5648_mode *mode)
{
	...
	mbus_format->ycbcr_enc = V4L2_MAP_YCBCR_ENC_DEFAULT(mbus_format->colorspace);
	mbus_format->quantization = V4L2_QUANTIZATION_FULL_RANGE;
	mbus_format->xfer_func = V4L2_MAP_XFER_FUNC_DEFAULT(mbus_format->colorspace);
}

static struct v4l2_mbus_framefmt *ov5648_get_pad_fmt(struct ov5648 *sensor,
					    struct v4l2_subdev_pad_config *cfg,
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	if (format->which == V4L2_SUBDEV_FORMAT_TRY){
		*mbus_format = *v4l2_subdev_get_try_format(subdev, config, format->pad);
	}
	else{
		ov5648_mbus_format_fill(mbus_format, sensor->state.mbus_code, sensor->state.mode);
	}
...
static const u32 ov5648_find_format(struct ov5648 *sensor, u32 code)
{
	unsigned int index;
	u32 mbus_code = ov5648_mbus_codes[0];//init with default code
	
	/* Try to find requested mbus code. */
	for (index = 0; index < ARRAY_SIZE(ov5648_mbus_codes); index++) {
		if (ov5648_mbus_codes[index] == code) {
			mbus_code = ov5648_mbus_codes[index];
			break;
		}
	}
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	/* Try to find requested mbus code. */
	mbus_code = ov5648_find_format(sensor, mbus_format->code);
...
static int ov5648_sensor_get_register(struct v4l2_subdev *sd,
				struct v4l2_dbg_register *reg)
{
	u8 val;
	int ret;

	ret = ov5648_read(sd, reg->reg & 0xff, &val);
	if (ret < 0)
		return ret;
...
static int ov5648_sensor_set_register(struct v4l2_subdev *sd,
				const struct v4l2_dbg_register *reg)
{
	return ov5648_write(sd, reg->reg & 0xff, reg->val & 0xff);
}
#endif
...
#ifdef CONFIG_VIDEO_ADV_DEBUG
	.g_register		= ov5648_sensor_get_register,
	.s_register		= ov5648_sensor_set_register,
#endif
};
...
static int ov5648_link_setup(struct media_entity *entity,
...
static int ov5648_suspend(struct device *dev)
{
	...
	ret = ov5648_sensor_power(subdev, 0);
	if (ret)
...
static int ov5648_resume(struct device *dev)
{
	...
	ret = ov5648_sensor_power(subdev, 1);
	if (ret)
		goto complete;
...
static void ov5648_set_defaults(struct ov5648 *sensor)
{
	sensor->formats = ov5648_formats;
	sensor->num_fmts = ARRAY_SIZE(ov5648_formats);
	sensor->modes = ov5648_modes;
	sensor->num_modes = ARRAY_SIZE(ov5648_modes);
	sensor->interval = ov5648_modes[0].frame_interval[0];

	sensor->fmt.code = sensor->formats[0].code;
	sensor->fmt.width = sensor->modes[0].output_size_x;
	sensor->fmt.height = sensor->modes[0].output_size_y;
...
static int ov5648_probe(struct i2c_client *client)
{
	...
	if (IS_ERR(sensor->avdd)) {
		dev_err(dev, "no AVDD regulator provided, using internal\n");
		sensor->avdd = NULL;
	}
	...
	dev_dbg(dev, "OmniVision OV5648 camera driver probed\n");
	return 0;
